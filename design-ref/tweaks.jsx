...
function TweaksPanel({ open, onClose, tweaks, setTweaks }) {
  ...
  return (
    <aside style={{
      position: "fixed", top: 20, right: 20, bottom: 20,
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
        <h3 className="display" style={{ fontSize: 20, margin: 0 }}>Tweaks</h3>
        <IconBtn icon={<Icons.X size={14}/>} onClick={onClose} size={28}/>
      </div>

      <Group label="테마">
        <Seg val={tweaks.theme} set={v => set("theme", v)} opts={[
      ...
      <Group label="포인트 컬러">
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 6 }}>
          {[
            ...
            <button key={o.id} onClick={() => set("accent", o.id)} style={{
      ...
      <Group label="정보 밀도">
        <Seg val={tweaks.density} set={v => set("density", v)} opts={[
      ...
      <Group label="주문 레이아웃 (주문 탭)">
        <Seg val={tweaks.queueLayout} set={v => set("queueLayout", v)} opts={[
          { id: "kanban", label: "칸반" },
          { id: "list",   label: "리스트" },
          { id: "card",   label: "카드" },
        ]}/>
      ...
      <Group label="AI 로그 표시">
        <Seg val={tweaks.agentLog} set={v => set("agentLog", v)} opts={[
...
function Group({ label, children }) {
  return (
    <div style={{ marginBottom: 18 }}>
      <div style={{ fontSize: 10, color: "var(--tx-2)", textTransform: "uppercase", letterSpacing: "0.12em", marginBottom: 8 }}>{label}</div>
      {children}
    </div>
  );
}
function Seg({ val, set, opts }) {
  return (
    <div style={{ display: "flex", background: "var(--bg-2)", border: "1px solid var(--line)", borderRadius: 8, padding: 3 }}>
      {opts.map(o => (
...
Object.assign(window, { TweaksPanel, DEFAULT_TWEAKS });
