// shell.jsx — Slim icon sidebar + topbar
const { useState: useStateS, useEffect: useEffectS } = React;

const NAV = [
  { id: "dashboard", icon: Icons.Home,     label: "대시보드" },
  { id: "orders",    icon: Icons.Orders,   label: "주문" },
  { id: "agent",     icon: Icons.Chat,     label: "AI 에이전트" },
  { id: "menu",      icon: Icons.Menu,     label: "메뉴" },
  { id: "sales",     icon: Icons.Sales,    label: "매출" },
  { id: "reviews",   icon: Icons.Reviews,  label: "리뷰" },
];
  ...
  return (
    <aside style={{
        ...
        {NAV.map(n => {
          ...
          return (
            <button key={n.id} onClick={() => onNav(n.id)} title={n.label}
              ...
              onMouseEnter={e => !isAct && (e.currentTarget.style.color = "var(--tx-0)", e.currentTarget.style.background = "var(--bg-1)")}
              onMouseLeave={e => !isAct && (e.currentTarget.style.color = "var(--tx-2)", e.currentTarget.style.background = "transparent")}
            >
              {isAct && (
                <span style={{
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
        <button title="도움말" style={iconBtnStyle()}><Icons.Help size={18}/></button>
        <button title="설정" onClick={() => onNav("settings")} style={iconBtnStyle(active === "settings")}>
          <Icons.Settings size={18}/>
        </button>
        <button title="로그아웃" style={iconBtnStyle()}><Icons.Logout size={18}/></button>
      </div>
    </aside>
  );
}
...
function Topbar({ onTweaks, page, storeStatus, setStoreStatus }) {
  const [now, setNow] = useStateS(new Date());
  const [menuOpen, setMenuOpen] = useStateS(false);
  const [confirmClose, setConfirmClose] = useStateS(false);
  useEffectS(() => {
    const t = setInterval(() => setNow(new Date()), 30000);
    return () => clearInterval(t);
  }, []);
  const titles = {
    dashboard: "대시보드", orders: "주문", agent: "AI 에이전트",
    menu: "메뉴", sales: "매출", reviews: "리뷰", settings: "설정",
  };
  ...
  return (
    <header style={{
      ...
      <h1 className="display" style={{
        fontSize: 26, margin: 0, color: "var(--tx-0)", letterSpacing: "-0.01em",
      }}>{titles[page]}</h1>
      <StoreStatusButton status={storeStatus} setStatus={setStoreStatus}
        menuOpen={menuOpen} setMenuOpen={setMenuOpen}
        onRequestClose={() => setConfirmClose(true)}/>
      <div style={{ flex: 1 }}/>
      {confirmClose && (
        <CloseConfirm onClose={() => setConfirmClose(false)}
          onConfirm={() => { setStoreStatus("closed"); setConfirmClose(false); }}/>
      )}

      <div style={{
        display: "flex", alignItems: "center", gap: 0,
        background: "var(--bg-1)", border: "1px solid var(--line)",
        borderRadius: 12, padding: "0 4px", height: 40,
      }}>
        <span style={{ padding: "0 14px", fontSize: 12, color: "var(--tx-1)", fontFamily: "var(--font-mono)", whiteSpace: "nowrap" }}>
          {time}
        </span>
        <span style={{ width: 1, height: 18, background: "var(--line-2)" }}/>
        <span style={{ padding: "0 14px", fontSize: 12, color: "var(--tx-1)", display: "flex", alignItems: "center", gap: 7, whiteSpace: "nowrap" }}>
      ...
      <div style={{
        ...
        background: "var(--bg-1)", border: "1px solid var(--line)",
        borderRadius: 12, padding: "4px 14px 4px 4px", height: 40,
      }}>
...
// ── Store Status Button (영업중 / 일시중지 / 종료) ────────────
function StoreStatusButton({ status, setStatus, menuOpen, setMenuOpen, onRequestClose }) {
  const cfg = {
  ...
  return (
    <div style={{ position: "relative" }}>
      <button onClick={() => setMenuOpen(v => !v)}
        ...
        <span style={{ display: "flex", flexDirection: "column", lineHeight: 1.15, textAlign: "left", whiteSpace: "nowrap" }}>
      ...
      {menuOpen && (
        <>
          ...
          <div className="fade" style={{
            ...
            <StatusOption status="open" current={status}
              label="영업 중" sub="신규 주문 자동 수락"
              dotC="var(--gn-500)"
              onClick={() => { setStatus("open"); setMenuOpen(false); }}/>
            <StatusOption status="paused" current={status}
              label="주문 일시중지" sub="10 ~ 60분 — 기존 주문만 처리"
              dotC="var(--yl-500)"
              onClick={() => { setStatus("paused"); setMenuOpen(false); }}/>
            <StatusOption status="closed" current={status}
              label="영업 종료" sub="오늘 더 이상 주문 받지 않음"
              dotC="var(--rd-500)"
              onClick={() => { setMenuOpen(false); onRequestClose(); }}
              warn/>
            <div style={{
              margin: "8px 4px 4px", padding: "8px 10px",
              background: "var(--bg-2)", borderRadius: 8,
              fontSize: 10.5, color: "var(--tx-2)", lineHeight: 1.5,
            }}>
...
function StatusOption({ status, current, label, sub, dotC, onClick, warn }) {
...
function CloseConfirm({ onClose, onConfirm }) {
  return (
    <div style={{
      ...
      <div className="fade" onClick={e => e.stopPropagation()} style={{
        width: 420, background: "var(--bg-1)", borderRadius: 18,
        border: "1px solid var(--line-2)", padding: 26,
        boxShadow: "var(--shadow-lg)",
      }}>
        <div style={{
          width: 44, height: 44, borderRadius: 12,
          background: "rgba(226,86,62,0.12)", color: "var(--rd-500)",
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <Btn variant="outline" size="md" style={{ flex: 1 }} onClick={onClose}>취소</Btn>
          <Btn variant="primary" size="md" style={{
            flex: 1.4,
            background: "var(--rd-500)", border: "1px solid var(--rd-500)",
            color: "#fff", boxShadow: "0 6px 18px rgba(226,86,62,0.3)",
          }} icon={<Icons.Power size={14}/>} onClick={onConfirm}>
            영업 종료
          </Btn>
        </div>
      </div>
