// ui.jsx — UI primitives for dark dashboard
...
function Card({ title, subtitle, eyebrow, action, children, padding, style, accent, noPad }) {
  return (
    <section style={{
      background: "var(--bg-1)",
          ...
          <div>
            ...
            {title && (
              <h3 className="display" style={{
                fontSize: 22, margin: 0, color: "var(--tx-0)", lineHeight: 1.1,
              }}>{title}</h3>
            )}
            {subtitle && (
...
// ── Button ────────────────────────────────────────────────────
function Btn({ children, variant = "ghost", size = "md", icon, iconRight, onClick, style, title, active }) {
  const sizes = {
  ...
  return (
    <button onClick={onClick} title={title}
      style={{
        ...
        gap: s.gap, height: s.h, padding: `0 ${s.px}px`, borderRadius: s.r,
...
// ── Pill / Tag ────────────────────────────────────────────────
function Pill({ children, tone = "default", style, soft, dot }) {
  const tones = {
...
function Live({ label, tone = "accent" }) {
  const colors = { accent: "var(--or-500)", green: "var(--gn-500)", red: "var(--rd-500)" };
  return (
    <span style={{
      display: "inline-flex", alignItems: "center", gap: 8,
      padding: "5px 11px 5px 9px", borderRadius: 999,
      background: "var(--bg-2)", border: "1px solid var(--line-2)",
      fontSize: 11, color: "var(--tx-1)", fontWeight: 500,
...
// ── Avatar ────────────────────────────────────────────────────
function Avatar({ name, size = 32, tone = "default" }) {
  const colors = {
...
// ── Progress bar ──────────────────────────────────────────────
function Bar({ value, height = 4, color = "var(--or-500)", track = "var(--bg-3)" }) {
  return (
...
function IconBtn({ icon, onClick, title, active, size = 36, style }) {
  return (
    <button onClick={onClick} title={title}
      style={{
      ...
      onMouseEnter={e => !active && (e.currentTarget.style.color = "var(--tx-0)", e.currentTarget.style.background = "var(--bg-2)")}
      onMouseLeave={e => !active && (e.currentTarget.style.color = "var(--tx-2)", e.currentTarget.style.background = "transparent")}
    >{icon}</button>
  );
}

Object.assign(window, { Logo, Card, Btn, Pill, Live, Avatar, Bar, Rating, IconBtn });
